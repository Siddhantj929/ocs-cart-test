...
const SearchBar = ({ shrink = false, className }: { shrink?: boolean, className?: string }) => {
    ...
    const isSearchBarActive = useMemo(() => activeSection !== null, [activeSection])
    ...
    const handleDateSelect = (selectedDate: DateRange | undefined, selectedDay: Date) => {
        if (!selectedDate) {
            setDate(undefined)
            return
        }
    ...
    return (
        <div className={cn(
            ...
            <div
                ...
                className={cn(
                    "w-64 h-16 rounded-full bg-background absolute top-0 left-0 origin-center scale-0 opacity-0 z-0 transition-[transform,scale,opacity] duration-300 ease-in-out",
